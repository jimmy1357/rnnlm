...
class PTBModel(object):
    ...
    def __init__(self, is_training, config):
        ...
        # self.cell = rnn.MultiRNNCell([rnn.BasicLSTMCell(size, forget_bias=0.0, state_is_tuple=True) for _ in range(config.num_layers)], state_is_tuple=True)
        self.cell = rnn.MultiRNNCell(lstm_cells, state_is_tuple=True)
        """
        self.cell = lstm_cell(size)
        if is_training and config.keep_prob < 1:
            self.cell = rnn.DropoutWrapper(self.cell, output_keep_prob=config.keep_prob)
        self.cell = rnn.MultiRNNCell([self.cell for _ in range(config.num_layers)], state_is_tuple=True)
        """
